...
inline const Node *
IntersectNodes(const Node *First, const Node *Second, auto PO,
               const std::map<const Node *, const Node *> &IDoms) {
  auto *Finger1 = First;
  auto *Finger2 = Second;
  while (Finger1 != Finger2) {
    auto Idx1 = GetIndexIn(Finger1, PO);
    auto Idx2 = GetIndexIn(Finger2, PO);
    while (Idx1 < Idx2) {
      Finger1 = IDoms.at(Finger1);
      Idx1 = GetIndexIn(Finger1, PO);
    }
    while (Idx2 < Idx1) {
      Finger2 = IDoms.at(Finger2);
      Idx2 = GetIndexIn(Finger2, PO);
    }
  }
  return Finger1;
}

inline std::map<const Node *, const Node *> ComputeIDom(const GraphTy &G) {
  ...
  while (Changed) {
    ...
    auto PickParent = [RPO](const auto *Nd) -> const Node * {
      for (const auto *Parent : Nd->Parents) {
        if (GetIndexIn(Parent, RPO) < GetIndexIn(Nd, RPO))
          return Parent;
      }
      return nullptr;
    };
